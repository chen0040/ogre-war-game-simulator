										...
										Vector3 &result)
{
	...
	return RaycastFromPoint(raySceneQuery, point, normal, result, excludedObjs);
}
...
bool OgreRayUtil::RaycastFromPoint(RaySceneQuery* raySceneQuery, const Vector3 &point,
                                        const Vector3 &normal,
										Vector3 &result, const std::set<MovableObject*>& excludedObjs)
{
    ...
    for (size_t qr_idx = 0; qr_idx < query_result.size(); qr_idx++)
    {
        ...
        {
			if(excludedObjs.find(query_result[qr_idx].movable) != excludedObjs.end())
			{
				continue;
			}
