...
Tank* STDAgentManager::createTank(const std::string& name, const std::string& modelName)
{
	if(mAgents.find(name) != mAgents.end())
	{
		throw Exception(42, "agent already exists!", "STDAgentManager::createTank(const std::string&, const std::string&)");
	}
	Tank* agnt=new Tank(name, modelName);
	mAgents[name]=agnt;
	return agnt;
}

URAV* STDAgentManager::createURAV(const std::string& name, const std::string& modelName)
{
	if(mAgents.find(name) != mAgents.end())
	{
		throw Exception(42, "agent already exists!", "STDAgentManager::createURAV(const std::string&, const std::string&)");
	}
	URAV* agnt=new URAV(name, modelName);
	mAgents[name]=agnt;
	return agnt;
}

Terrorist* STDAgentManager::createTerrorist(const std::string& name, const std::string& modelName)
{
	if(mAgents.find(name) != mAgents.end())
	{
		throw Exception(42, "agent already exists!", "STDAgentManager::createTerrorist(const std::string&, const std::string&)");
	}
	Terrorist* agnt=new Terrorist(name, modelName);
	mAgents[name]=agnt;
	return agnt;
}
